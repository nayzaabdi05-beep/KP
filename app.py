...
if menu == "1. Overview & Data Cleaning":
    ...
    st.markdown("Analisis distribusi lama waktu pemenuhan material serta tren transaksi secara umum.")
    ...
    st.markdown("Bagian ini menyajikan rincian jumlah transaksi, rata-rata lead time, serta *breakdown* kategori lead time (Cepat, Standar, Lambat) untuk setiap bulan.")

    # Agregasi data per bulan
    df_bulanan = df.groupby("Bulan_MR").agg(
        Total_Transaksi=("Part ID", "count"),
        Rata_Rata_Lead_Time=("Rentang_Hari", "mean")
    ).reset_index()

    pivot_kategori = pd.crosstab(df["Bulan_MR"], df["Kategori_LeadTime"])
    rekap_df = pd.merge(df_bulanan, pivot_kategori, on="Bulan_MR")
    rekap_df["Rata_Rata_Lead_Time"] = rekap_df["Rata_Rata_Lead_Time"].round(2)

    col1, col2 = st.columns(2)
    col1.metric("Jumlah Bulan Teranalisis", len(rekap_df))
    col2.metric("Total Keseluruhan Transaksi", len(df))

    st.markdown("---")

    st.subheader(" Tabel Rekapitulasi Bulanan")
    st.dataframe(rekap_df, use_container_width=True)

    st.subheader(" Tren Kategori Lead Time Berdasarkan Bulan")
    fig, ax = plt.subplots(figsize=(10, 5))
    
    # Pastikan kolom kategori lengkap jika ada yang kosong
    ...
    st.title("🏷️ 4. Pelabelan Kategori Lead Time")
    st.markdown("""
