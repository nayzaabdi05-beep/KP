...
# ==========================================
# 3. SIDEBAR & GLOBAL FILTER BULAN
# ==========================================
st.sidebar.title(" Panel Kontrol")
st.sidebar.markdown("---")

# Opsi Filter Bulan
daftar_bulan = sorted(df_raw["Bulan_MR"].unique())
pilihan_bulan = st.sidebar.selectbox(
    " Filter Periode Bulan:",
    options=["Overall (Semua Bulan)"] + list(daftar_bulan)
)

# Terapkan Filter Berdasarkan Pilihan Sidebar
if pilihan_bulan == "Overall (Semua Bulan)":
    df = df_raw.copy()
else:
    df = df_raw[df_raw["Bulan_MR"] == pilihan_bulan].copy()
...
menu = st.sidebar.selectbox(
    ...
    (
        ...
        "3. Analisis Data per Bulan",
        "4. Pelabelan Lead Time",
        "5. Model Training & SMOTENC",
        "6. Evaluasi & Overfitting Check",
        "7. Prediksi Real-Time Mandiri"
    )
)

st.sidebar.markdown("---")
st.sidebar.info(f" **Status Filter:** {pilihan_bulan} ({len(df)} baris data aktif)")
...
if menu == "1. Overview & Data Cleaning":
    st.title(" 1. Overview & Data Cleaning")
    st.markdown(f"Menampilkan ringkasan data bersih untuk periode: **{pilihan_bulan}**.")

    col1, col2, col3 = st.columns(3)
    col1.metric("Total Data Aktif", len(df))
    col2.metric("Jumlah Fitur Utama", len(df.columns))
    col3.metric("Periode Terpilih", pilihan_bulan)
    ...
    st.markdown(f"Analisis distribusi untuk filter: **{pilihan_bulan}**.")
    ...
    with col1:
        ...
        ax.set_title(f"Distribusi Lead Time ({pilihan_bulan})", color="white")
    ...
    with col2:
        ...
        if pilihan_bulan == "Overall (Semua Bulan)":
            ...
            # Jika dipilih 1 bulan spesifik, tampilkan ringkasan kategori/hari
            harian = df.groupby(df["MR Date"].dt.date).size().reset_index(name="Jumlah")
            sns.lineplot(data=harian, x="MR Date", y="Jumlah", ax=ax, marker="o", color="#10b981")
            ax.set_title(f"Tren Harian di Bulan {pilihan_bulan}", color="white")
            plt.xticks(rotation=45)
        
        ax.set_xlabel("Tanggal / Bulan", color="white")
        ax.set_ylabel("Jumlah Transaksi", color="white")
        fig.patch.set_facecolor('#0e1117')
        ax.set_facecolor('#1f2937')
        st.pyplot(fig)

elif menu == "3. Analisis Data per Bulan":
    st.title(" 3. Analisis & Rekapitulasi Data Per Bulan")
    st.markdown("Bagian ini menyajikan rincian jumlah transaksi, rata-rata lead time, serta *breakdown* kategori lead time.")

    # Agregasi data per bulan (menggunakan df_raw agar perbandingan bulanan tetap utuh meskipun difilter, atau df aktif)
    df_bulanan = df_raw.groupby("Bulan_MR").agg(
        Total_Transaksi=("Part ID", "count"),
        Rata_Rata_Lead_Time=("Rentang_Hari", "mean")
    ).reset_index()

    pivot_kategori = pd.crosstab(df_raw["Bulan_MR"], df_raw["Kategori_LeadTime"])
    rekap_df = pd.merge(df_bulanan, pivot_kategori, on="Bulan_MR")
    rekap_df["Rata_Rata_Lead_Time"] = rekap_df["Rata_Rata_Lead_Time"].round(2)

    col1, col2 = st.columns(2)
    col1.metric("Status Filter Aktif", pilihan_bulan)
    col2.metric("Total Transaksi Terfilter", len(df))

    st.markdown("---")
    st.subheader(" Tabel Rekapitulasi Keseluruhan Bulan")
    st.dataframe(rekap_df, use_container_width=True)

    st.subheader(" Tren Kategori Lead Time Berdasarkan Bulan")
    fig, ax = plt.subplots(figsize=(10, 5))
    
    for cat in ["Cepat", "Standar", "Lambat"]:
        if cat not in pivot_kategori.columns:
            pivot_kategori[cat] = 0
    pivot_kategori = pivot_kategori[["Cepat", "Standar", "Lambat"]]

    pivot_kategori.plot(kind="bar", stacked=True, ax=ax, color=["#10b981", "#f59e0b", "#ef4444"])
    
    ax.set_title("Distribusi Kategori Lead Time per Bulan", color="white")
    ax.set_xlabel("Bulan MR", color="white")
    ax.set_ylabel("Jumlah Transaksi", color="white")
    plt.xticks(rotation=0)
    ax.legend(title="Kategori", facecolor="#1f2937", labelcolor="white")
    
    fig.patch.set_facecolor('#0e1117')
    ax.set_facecolor('#1f2937')
    st.pyplot(fig)

elif menu == "4. Pelabelan Lead Time":
    st.title(" 4. Pelabelan Kategori Lead Time")
    st.markdown(f"Distribusi kategori lead time untuk data terfilter: **{pilihan_bulan}**.")

    urutan = ["Cepat", "Standar", "Lambat"]
    kategori = df["Kategori_LeadTime"].value_counts().reindex(urutan).fillna(0)
...
elif menu == "5. Model Training & SMOTENC":
    st.title(" 5. Train-Test Split & SMOTENC Pipeline")
    st.markdown("Pelatihan model tetap menggunakan dataset keseluruhan (`df_raw`) agar pola data terjaga secara konsisten.")

    features = ["Part ID", "Unit", "WO_MR_Days", "MR_Month", "MR_DayOfWeek"]
    X = df_raw[features]
    y = df_raw["Kategori_LeadTime"]
...
elif menu == "6. Evaluasi & Overfitting Check":
    st.title(" 6. Evaluasi Random Forest & Cek Overfitting")
    ...
    with col_left:
        ...
        st.info(" **Analisis**: Terdapat gap performa antara data training dan testing yang mengindikasikan sedikit overfitting.")
...
elif menu == "7. Prediksi Real-Time Mandiri":
    st.title(" 7. Form Prediksi Lead Time Transaksi Baru")
    st.markdown("Masukkan parameter transaksi material di bawah ini untuk memprediksi kategori lead time secara langsung.")
    ...
    with st.form("prediction_form"):
        col1, col2 = st.columns(2)
        with col1:
            part_id_input = st.selectbox("Part ID", options=sorted(df_raw["Part ID"].unique()))
            unit_input = st.selectbox("Unit", options=sorted(df_raw["Unit"].dropna().unique()))
